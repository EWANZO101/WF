...
class Service implements ServiceInterface
{
    ...
    // Buttons
    public static function testConnection(): RedirectResponse
    {
        $type = 'error';
        $url = settings('encrypted::pterodactyl::api_url', '');
        try {
            $url_accept = '<p>[URL] is ' . (Http::head($url)->successful() ? 'available' : 'not available');
        } catch (Exception $e) {
            $url_accept = $e->getMessage();
        }

        // SSO
        $sso = self::checkSsoAuthorization($url, settings('encrypted::pterodactyl::sso_secret', ''));

        // Client
        $client = new Pterodactyl(settings('encrypted::pterodactyl::api_admin_key', ''), $url);
        $client_message = '<p>[Client API] Failed to connect to client. ';
        if ($client->checkAuthorizationClient()) {
            $client_message = '<p>[Client API] is available. ';
        }
        ...
        try {
            ...
            if ($api->checkAuthorization()) {
                $type = 'success';
                $message = '<p>[Application API] is available. ' . $client_message . ' ' . $url_accept . ' ' . $sso;
            } else {
                $message = '<p>[Application API] Failed to connect to api. ' . $client_message . ' ' . $url_accept . ' ' . $sso;
            }
        } catch (Exception $e) {
            $message = $e->getMessage() . ' ' . $sso;
        }
        return redirect()->back()->with($type, $message);
    }

    public static function changePteroPassword(Order $order, string $newPassword)
    {
        return ptero()->user()->changePassword($order, $newPassword);
    }

    private static function checkSsoAuthorization($url, $secret)
    {
        ...
        if ($sso->successful() and is_array($sso->json())) {
            return array_key_exists('message', $sso->json()) ? $sso->json()['message'] : '<p>[SSO] Authorized';
        } elseif ($sso->getStatusCode() == 501) {
            return ($sso->json()['message'] == 'You cannot automatically login to admin accounts.' ? '<p>[SSO] Authorized' : $sso->json()['message']) ?? '<p>[SSO] Unknown error';
        }
        return '<p>[SSO] Not Authorized';
    }
